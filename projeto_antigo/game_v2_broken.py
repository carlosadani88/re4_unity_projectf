...
# BUILD 3D WORLD
# ═══════════════════════════════════════════════════════════
# Ground — single textured plane
ground = Entity(
    model='plane',
    scale=(M_W, 1, M_H),
    position=(M_W/2, 0, M_H/2),
    texture=tex_ground,
...
# Sky dome — overcast RE4 mood
sky_ent = Sky(texture=tex_sky if tex_sky else None,
              color=color.rgb(72, 65, 55) if not tex_sky else color.rgb(160, 150, 135))

# Fog — thick village atmosphere
scene.fog_color = color.rgb(58, 52, 42)
scene.fog_density = 0.035

# Ambient light
dl = DirectionalLight()
dl.look_at(Vec3(1, -1, 1))
    ...
    if 0 < sx < M_W and 0 < sz < M_H and get_tile(sx, sz) == 0:
        if kind == 'barrel':
            ...
            Entity(model=get_cylinder(), position=(sx, 0.4, sz),
                   scale=(0.8, 0.8, 0.8), color=color.rgb(65,60,50))
            Entity(model=get_cylinder(), position=(sx, 0.05, sz),
                   scale=(0.6, 0.1, 0.6), color=color.rgb(15,18,28))
